...
- プーリング方法の選び方：
    - 最大プーリング（Max Pooling）：
        ...
        - 余談：畳み込み（画像パッチとカーネルの内積）が大きくなる方向にカーネルが更新されていくのはなぜ？それは，勾配降下法における更新式が「入力画像パッチが$x$の時，損失の微分が負なら，$x$に依存した更新量をカーネルに**足す**．」「入力画像パッチが$x$の時，損失の微分が正なら，$x$に依存した更新量をカーネルから**引く**．」という挙動をするから．つまり直感的には，ランダムだったカーネル$w$は無数のエポックの後，以下のように化ける．

            $w \approx \sum (\text{役に立つ画像パッチ}) - \sum (\text{役立たずの画像パッチ})$

            より正確には，

            $$
            \begin{align*}
                w &= w_0 - \sum (\eta \frac{\partial L}{\partial w}) \\
                &= w_0 - \sum (\eta \frac{\partial L}{\partial y} \frac{\partial y}{\partial w}) \\
                &= w_0 - \sum (\eta \frac{\partial L}{\partial y} \frac{\partial (w\cdot x)}{\partial w}) \\
                &= w_0 - \sum (\eta \frac{\partial L}{\partial y} x)
            \end{align*}
            $$
            ...
            - 「役に立つ」＝ $\frac{\partial L}{\partial y} < 0$
            - 「役立たず」＝ $\frac{\partial L}{\partial y} > 0$

            である．
        
            例え話をしよう．警察が「犯人のモンタージュ写真」を作るシーンを想像してね．
            - 初期状態（ランダムなカーネル）：最初は，白紙に適当な線を引いたようなデタラメな似顔絵$w$を持っている．
            - 役に立つ画像パッチ（犯人の目撃情報）：「犯人はこんな目をしていました！」という写真（入力$x$）が来た．損失関数は「似顔絵にこの特徴を足せ（微分が負）」と指示する．似顔絵の目は，目撃情報に寄っていく．
            - 役立たずの画像パッチ（無関係な人の情報）：「この人は無実の一般人です．」という写真（入力$x$）が来た．損失関数は「似顔絵からこの特徴を引け（微分が正）」と指示する．似顔絵から，その一般人と同じ特徴を消しゴムで消す．

            これを何万枚もの写真で繰り返すと，デタラメだった似顔絵は「無実の人々の特徴が引き算され，犯人の特徴だけが足し算された，完璧なモンタージュ写真」に化ける．これが，カーネルの正体だ．
