...
## 全ての議論の前に感じるべし「生成モデルが存在する意義」
- 結論

    生成モデルの存在意義は，**学習データと同じ意味を持ち，かつ新しいデータを作る事！**

- 学習データと完全に同じデータが欲しいなら

    AIなんて使わなくてよい．HDDにファイルを保存して，ランダムなインデックスで呼び出すだけの「データベース」を作れば済む話だ．

- 新しいデータを作る方法の1つは分布からサンプリングする事

    もしデータベースなら，コードはこうなる．

    `x = dataset[random.randint(0, 9999)]`

    しかし，生成モデルの推論コードはこう始まる．

    `z = torch.randn(1, latent_dim)`

    インデックス番号の代わりに，**無限の解像度を持つ連続的な実数空間からサンプリング**する事．これこそが「既存のデータのコピーではなく，新規のデータを生み出す」というモチベーションが具現化した姿なんだ．

- 「学習データと同じ意味を持つ」という言葉の定量的な表現

    - $x$：データ（例えば猫画像）
    - $p_{data}(x)$：真のデータ分布．この世界に存在する「ありえる猫画像」の確率分布．神のみぞ知る完璧な雲．
    - $p_{\theta}(x)$：モデルの分布．ニューラルネット等が，重み$\theta$によって作り上げた確率分布．

    $p_{data}(x)$がわかれば世話ないのだけど，それは無理だ．1次元の身長データの分布を観察するのでさえ，数万サンプルが必要だったよね．とすれば，$512*512*3$の画像（約80万次元！）での議論になると，サンプル数が足りるはずがない．これが「次元の呪い」だ．

    だから，生成モデルは，**$p_{\theta}(x)$を$p_{data}(x)$に限りなく近づける事**を究極の目的とする．数式での「同じ確率分布にする」が，自然言語での「学習データと同じ意味・ルールを持つようにする」なんだよ．

- いつも聞いていたあの言葉

    学習データと同じ意味を持つ新しいデータを作る能力を「汎化性能」と呼ぶ．対義の状態は，「過学習」と呼ぶ．データベースは，過学習の神様だね．

## 拡散モデルの拡散過程で最後は「完全なノイズ」にならないといけないのは何故？
- 結論

    後でイチから新しい画像を生成する時の「スタート地点」を，誰でも再現可能なものにしたいから！

- では「完全なノイズ」とはなんだろう？

    - 【最重要】共分散行列が単位行列$I$である事！

        共分散行列が単位行列であるとは，画像で言えば「ピクセル同士が完全に独立（無相関）である」事を意味する．相関があると，モデルを使う人が相関を加味した職人芸な初期の入力（ノイズ）を作らなければならないのだ．
    
    - 標準化されている事（平均0・分散1）

        これは，どちらかと言えばディープラーニング的要請によるものだ．学習が遅々として進まない事態や，勾配爆発によりモデル内を流れるテンソルの値が無限大になる事，活性化関数がサチって勾配消失を起こす事を防ぐために，いつも通り標準化しよう．

## 拡散過程の式に出現するルートの意味
$$x_t = \sqrt{1 - \beta_t}x_{t-1} + \sqrt{\beta_t}\epsilon$$
どうしてルートがついているんだろう？係数の和が1なら良さそうじゃない？

結論は，**拡散過程を通して分散を変えずに維持するため！**

計算してみよう．

- $x_{t-1}$：1つ前のステップの画像．分散は $V(x_{t-1}) = 1$ に保たれていると仮定する．
- $\epsilon$：追加するノイズ．標準正規分布からサンプリングされるため，分散は $V(\epsilon) = 1$．
- $\beta_t$：ノイズのスケジュール（例：$0.01$ などの小さな定数）．
- 【超重要ルール1】 変数 $X$ を $a$ 倍すると，ばらつき（分散）は $a^2$ 倍になる．：$V(aX) = a^2 V(X)$
- 【超重要ルール2】 無関係な変数同士を足すとき，分散はそのまま足し算になる．：$V(X + Y) = V(X) + V(Y)$

1. ルートをつけない場合（失敗例）
    $$x_t = (1 - \beta_t)x_{t-1} + \beta_t\epsilon$$
    分散の公式を適用する．
    $$V(x_t) = (1 - \beta_t)^2 V(x_{t-1}) + (\beta_t)^2 V(\epsilon)$$
    $V(x_{t-1}) = 1$ と $V(\epsilon) = 1$ を代入する．
    $$V(x_t) = (1 - \beta_t)^2 + \beta_t^2 = 1 - 2\beta_t + 2\beta_t^2$$
    これでは，$\beta_t = 0, 1$以外において，$V(x_t) \neq 1$となってしまう．$\beta_t = 0, 1$では拡散過程が作れないし，$\beta_t \neq 0, 1$では分散が潰れるか発散してしまう．標準化された状態が維持できないんだね．

2. ルートをつける場合（大成功）
    $$x_t = \sqrt{1 - \beta_t}x_{t-1} + \sqrt{\beta_t}\epsilon$$
    $$V(x_t) = (\sqrt{1 - \beta_t})^2 V(x_{t-1}) + (\sqrt{\beta_t})^2 V(\epsilon)$$
    $$V(x_t) = (1 - \beta_t)(1) + \beta_t(1)$$
    $$V(x_t) = 1 - \beta_t + \beta_t = 1$$
    お見事！

## 拡散モデル：逆拡散過程と実装のリアル

### 1. ゼロ作⑤（179-180ページ）のまとめ
逆拡散過程（ノイズから画像を削り出す手順）は，VAEのDecoderと同じく「確率分布」として定義される．

- 役割：ニューラルネットワークは，ノイズ画像 $x_t$ と時刻 $t$ を受け取り，1ステップ前の画像の「推定値（平均）$\hat{x}_{t-1}$」を出力する．
- 数式：$p_\theta(x_{t-1} | x_t) = \mathcal{N}(x_{t-1} ; \hat{x}_{t-1}, \mathbf{I})$
- 意味：ネットワークの出力を「平均」とし，そこに少しだけノイズ（分散）を足した雲の中から，次のステップの画像 $x_{t-1}$ をサンプリングする．

### 2. プロの実装（DDPM等）における重大な変更点
教科書の理論上は「1ステップ前の画像 $\hat{x}_{t-1}$」を予測するが，**実際の実装では「画像」ではなく「ノイズ $\epsilon$」を予測させる．**

- 理由：画像全体をイチから描き直すより，元の画像との差分である「直前で足されたノイズの模様」だけを見つけ出して予測する方が，ニューラルネットワークにとってタスクが圧倒的に簡単であり，生成品質も劇的に向上するため．
- U-Netの活躍：出力するのは「画像」ではなく「純粋なノイズ」である．U-Netのスキップ接続は，圧縮によって潰れてしまう「ピクセル単位の細かいザラザラ情報（浅い畳み込み層による特徴マップ）」を後半にワープさせる役割を持つ．深い層で「画像の大局的な構造」を認識し，それをヒントにしてスキップ接続で送られてきた情報から「画像成分を捨ててノイズ成分だけを抽出する」という高度なフィルタリングを行っている．

### 3. 推論（生成）プロセスの疑似コード
実装上の逆拡散過程（$x_t \rightarrow x_{t-1}$）は，以下のステップの繰り返しで実行される．

```python
# 1. ネットワークは画像ではなく「ノイズ」を予測する
predicted_noise = model(x_t, t)

# 2. 現在の画像 x_t からノイズを引き算し，1ステップ前の平均値（x_t_minus_1_hat）を計算する
# （※実際はここに論文で定義されたスケール調整の係数が掛かる）
x_t_minus_1_hat = (x_t - coef1 * predicted_noise) / coef2

# 3. 予測した平均値に，分散分のノイズを足してサンプリング（180ページの数式の具現化）
noise = torch.randn_like(x_t)
x_t_minus_1 = x_t_minus_1_hat + coef3 * noise
```

VAEの出力では，砂嵐になることを嫌って「平均値であるNNの出力をそのまま使おうね」とお話したのに，ここでは最後にサンプリングしているのは何故だろうか．これは，「出力がのっぺらぼうになるのを防ぐため」だ．VAEをMNISTで動かした時の結果は，なんだか平滑化フィルタがかかったような，高周波成分の無い画像になっていたよね．これを，DDPMの逆拡散過程で1000回も繰り返すと，てろてろのディティールが欠如した出力になってしまうよ．ただし，DDPMでも最終出力だけはサンプリングせずに平均値をそのまま採用するんだ．

（※途中のステップでもサンプリングを行わずに，完全に決定論的に平均値だけで進むDDIMという拡散モデルも存在するらしい．）

## 拡散モデル最大の錯覚：「ノイズ予測」の本当の意味

### 1. 錯覚の正体：「未来の予測」ではなく「現在の分離」
- 僕の錯覚：綺麗な画像に対して「これからどんなランダムなノイズが足されるか」をエスパー予測している．
- 拡散モデルの真実：「すでにノイズが混ざって汚れた画像（$x_t$）」を入力とし，その中から「今乗っているノイズ成分（$\epsilon$）だけを抽出・分離」している（コーヒーのシミ抜きと同じ）．

### 2. 学習時の実装と数式の真実
ネットワークが受け取る $x_t$ の中には，すでに正解となるノイズ $\epsilon$ が物理的に足し算されて含まれている．
- 数式：$\text{LOSS} = ||\epsilon_\theta(x_t, t) - \epsilon||^2$
- 意味：「未知の未来を当てろ」ではなく，「$x_0$ と $\epsilon$ が混ざった $x_t$ をヒントとして渡すから，その中から $\epsilon$ を逆算して抜き出しなさい」というタスクである．

### 3. なぜ画像とノイズを分離できるのか？（空間的な性質の違い）
画像とノイズが足されて1つの数字になっていても，2次元の「空間的な性質（周波数特性）」が全く異なるため，ネットワークは両者を見分けることができる．
- 画像（$x_0$）の性質：ピクセル同士に強い相関がある（隣のピクセルと似た色になりやすい）．低周波成分が多く，滑らかで規則的なエッジを持つ．
- ノイズ（$\epsilon$）の性質：ピクセル同士が完全に独立・無相関である．高周波成分の塊であり，不規則でザラザラしたスパイクの集まりである．

### 4. 畳み込み（CNN）は性質の違いをどう捉えるか
U-Netのような畳み込みニューラルネットワーク（CNN）は，画像に小さなフィルタ（カーネル）をスライドさせることで，この空間的な違いを直接捉える．
- 周波数フィルタとしての機能：学習された畳み込みフィルタの中には，「値が滑らかに変化する部分（画像成分）」を抽出する平滑化フィルタのようなものと，「値が急激にギザギザ変化する部分（ノイズ成分）」に強烈に反応するエッジ検出のようなものが自然に形成される．
- U-Netの連携：深い層で「これは顔だ，背景だ」という大局的な構造（低周波）を認識し，浅い層のSkip Connectionで「このザラザラは不自然だからノイズだ」という局所的な判断（高周波）を行う．これにより，画像成分を無視してノイズ成分だけを精緻に削り出すことが可能になる．

## 正弦波位置エンコーディングの理解ノート

### 1．そもそものモチベーション：なぜ整数のままではダメなのか
拡散モデルの時刻tやTransformerのパッチ位置など，ニューラルネットワークに順番や位置を教える必要がある．しかし，時刻tをそのまま入力することには致命的な欠点がある．
- 整数の暴走：t=1000などの大きな値をそのまま入れると，ネットワークの重みが破壊される．
- 正規化の限界：0.0から1.0にスケールを縮めると，t=1とt=2の差が微小になりすぎてAIが見分けられなくなる．
結論：1つのスカラー値ではなく，複数の数値の組み合わせ（ベクトル）で位置を表現する必要がある．

### 2．直感的な理解：AIのための精密なアナログ時計
正弦波位置エンコーディングの数式は，回るスピード（周期）が異なる複数の針を持つアナログ時計を作っている．
ここで最も重要な法則は，位置を表すベクトルの要素は必ず「sinとcosの2個で1セット（ペア）」になるということである．時計の針の角度を表現するにはx座標とy座標の両方が必要なように，同じ周波数（分母）を持つsinとcosのペアが1つの針を構成する．
- 低次元のペア（分母が小さい）：超高速で回る秒針．細かい時間の違いに敏感に反応する．
- 高次元のペア（分母が巨大）：超ゆっくり回る時針．全体のどのあたりにいるかという大局的な位置を把握する．
次元の数だけ時計の針のペアを用意し，その角度の組み合わせで現在時刻を表現する．

<p align="center">
    <img src="./images/positional_encoding.gif" width="500">
</p>

### 3．定義式とペアの共有
「Attention Is All You Need」における位置エンコーディング（PE）の厳密な定義式は以下の通りである．位置をpos，次元のインデックスをi，全体の次元数をdとする．

$$PE(pos, 2i) = sin(pos / 10000^{2i/d})$$
$$PE(pos, 2i+1) = cos(pos / 10000^{2i/d})$$

この式が示す通り，偶数インデックス（2i）と奇数インデックス（2i+1）は，全く同じ分母（周波数）を共有してペアになっている．この数学的な美しさが，次の相対距離の計算を可能にする．

### 4．最大の数学的メリット：相対距離の計算と導出
ランダムな値ではなくsinとcosのペアを使う理由は，AIが相対距離（引き算）を瞬時に計算できるようにするためである．
ある周波数 ω = 1 / 10000^(2i/d) のペアに注目し，現在位置 t から Δt だけ離れた位置 t + Δt のエンコーディングを考える．高校数学の加法定理を展開すると，以下のような行列のかけ算として導出できる．

$$
\begin{bmatrix}
\sin\!\bigl(\omega (t+\Delta t)\bigr) \\
\cos\!\bigl(\omega (t+\Delta t)\bigr)
\end{bmatrix}
=
\begin{bmatrix}
...
この式の右辺の変換行列（2×2の行列）に注目すると，現在位置 t が全く含まれず，移動させたい距離 Δt のみに依存していることがわかる．
ニューラルネットワークは行列のかけ算（線形変換）が得意なため，現在時刻 t が何であっても関係なく，ある一定の距離 Δt だけ離れた特徴を捉えるための重み（変換行列）を容易かつ汎用的に学習できるようになる．

## DDPMにおける正弦波ベクトルを変換するMLPの役割

### 1．MLPの本当の仕事：「時計の読み取り」と「指示出し」
U-NetのConvBlockに組み込まれたMLPの目的は，Transformerのように要素間の相対距離（Δt）を計算することではない．最大の役割は，正弦波エンコーディング（時計の針）を読み取り，「今の時刻（ノイズレベル）なら，U-Netのどのチャネル（フィルター）を強く働かせるべきか」という指示書（バイアス）を作成することである．
- 序盤（ノイズが多い）：MLPは「大きな輪郭を探せ」と判断し，低周波成分を捉えるチャネルを活性化させるベクトルを出力する．
- 終盤（ノイズが少ない）：MLPは「細かい質感を整えろ」と判断し，高周波のテクスチャを生成するチャネルを活性化させる．
コードにおける x + v という処理は，時間帯に応じた画像フィルターのスイッチ（条件付け）を物理的に切り替える役割を果たしている．

### 2．正弦波とMLPの相性の良さ（イコライザー効果）
時刻tをそのまま単一の数値としてMLPに入力しても，ニューラルネットワークは微小なステップの違い（例：ステップ500と501の違い）をうまく学習できない．
しかし，時間を正弦波の「高周波から低周波までの波（秒針から時針）」に分解して入力することで，MLP内の線形層はオーディオのイコライザーのように各成分へ適切に重みを割り振ることができるようになる．
- 高周波の成分：1ステップごとの細かい微調整の指示を出す．
- 低周波の成分：序盤，中盤，終盤といった大雑把なフェーズの指示を出す．

### 3．結論
拡散モデルにおいて正弦波位置エンコーディングとMLPを組み合わせる最大のモチベーションは，1次元の時間をMLPが解釈しやすい多次元の周波数帯域へ変換し，U-Netが「ノイズ除去の進行度」に応じて適切な画像処理フィルターを動的に切り替えられるようにすることである．
...
1. 画像をパッチへ分割
    ...
    パッチへ分割した後は，空間次元もチャネル次元もまとめて大胆にflatten！

2. パッチの埋め込み

    - 埋め込み（Embedding）：データをベクトルで表現する事．（特徴ベクトルと埋め込みベクトルに数学的な違いは無いと思う．特徴表現の役割で見るか，変換の役割で見るかによって名前が変わっているだけと認識している．）

    パッチのベクトルに埋め込みを行う．ベクトルをベクトルに変換するのだから，「より良いベクトル」に変化していなければならない．膨大な単語数や巨大な次元において人間がそれを決めるのは不可能なので，ニューラルネットワークに丸投げする．

    **「より良いベクトル」⇔「損失を最小化するベクトル」**

    これをViTでは，入力を出力に直接写像する1つの全結合層に学習させる．画像の位置ではなく文脈を翻訳したいので，全パッチに対して共通のネットワークを適用する．（一般にConv2dで実装する．）

3. クラストークン

    全てのパッチの埋め込みベクトルの先頭に，埋め込みベクトルと同じ長さの「クラストークン」をポン付けする．ココに後述のSelf-Attentionによって画像全体の情報が凝縮される．

4. 位置埋め込み

    正弦波位置エンコーディングなら分かるけど，ただのランダムな初期値のベクトルを学習させて，それがなぜ勝手に「位置情報」として機能するようになるのか？

    > なぜ、ただのランダムな初期値から始まったパラメータが、最終的に「位置」という意味を持つようになるのか？それは、「Attention層が損失関数を下げるために、位置情報を必要として推測するから」です。

    > 実際に学習が終わった後の $E_{pos}$ のベクトルを取り出し、それぞれのパッチ間のコサイン類似度（どれくらいベクトルが似ているか）を計算してヒートマップにするとどうなるか。なんと、綺麗な「2次元のグリッド構造（縦横の座標系）」が自然に浮かび上がります。 左上のパッチの位置ベクトルは、その周囲のパッチの位置ベクトルとだけ強い類似度を持つように「勝手に整理整頓」されているのです。

    <p align="center">
        <img src="./images/position_embedding.png" width="750">
    </p>

    Geminiによれば，データが少なすぎると位置埋め込みが失敗するかも？
    > ViTの原論文で、あの美しい2Dグリッド構造が浮かび上がったのは、「JFT-300M」という3億枚の非公開画像データセットを使って、Googleの巨大なサーバーで力技で学習させたからです。
    > もしあなたが、研究室のPCを使って「1万枚」程度の小さな画像データセットでViTをゼロから学習させたとします。すると、位置埋め込みは2D構造を見つけ出せず、ただのノイズのまま学習が終了します（そして精度はCNNに惨敗します）。
    > 「人間が必死こいて位置を定式化するより学習させた方が損失が下がる」というのは、「無限に近いデータと計算資源があれば」という厳しい条件付きなのです。だからこそ、最新の研究（例えばLLMにおけるRoPEなど）では、「完全な白紙からの学習」と「人間による数学的な定式化（正弦波など）」の良いとこ取りをするアプローチが主流になっています。

### Self-Attention
- 全体の流れ

    Self-Attentionは，「見た目は全然違っても構わないけど，意味的には強い関係がある人たちを集めて，より良い自分になる」事を目標とする．

    各パッチは，
    1. 全パッチの情報を抽出（埋め込み）
    2. 自分自身も含めて全パッチとの類似度を計算（内積）
    3. 類似度に基づいて合体（加重和）

    を行う．同じ事を演算の観点からもう一度書いてみる．

    1. 埋め込み：別々の線形層によってクエリ・キー・バリューを作成
    2. 内積：クエリと転置キーの行列積によってパッチ同士の類似度を計算
    3. 加重和：Attention WeightでValueを加重和し，各パッチの新しい特徴表現を生成する．誤差逆伝播を通じて，この特徴表現が損失を小さくするようにQuery・Key・Valueの生成パラメータが更新される．
...
- MHSA (Multi-Head Self-Attention)
    ...
    - わざわざマルチヘッドにする理由：
        ...
        では，ヘッドの埋め込み次元を1にしちゃえば最強なのか？いいえ．1次元埋め込みでは，複雑な関係を計算できなくなる．
        
        これはトレードオフの話である．

        - ヘッド数：多ければ「多様な視点」を持てる．（同じ役割のヘッドも生じるかもしれない．自由度が増えるくらいに捉えよう．）
        - ヘッド次元：大きければ「複雑な類似度」を計算できる．

        Single Headは，ヘッド数をガン無視して，ヘッド次元に全振りしていた状態であった．Single Headは，あくまでもMulti-Headの特別な場合であっただけの話である．
        
        どんな分割が最適かは，タスクなどに依存すると思われ，ハイパーパラメータ調整の領域である．

    - ソフトマックス関数へ入力する前に「ヘッドの埋め込み次元数の平方根」でAttention Weightを除算する理由：

        ソフトマックス関数は，指数関数の性質により，入力値が大きい時入力値の差に敏感であり，極端な重みの分配を行ってしまう．これが，「重みの勝者総取り」状態を作り，勾配消失問題を引き起こすためである．

        $q$と$k$という2つのベクトルがあり，それぞれの成分が平均$0$，分散 1$の独立した確率変数であると仮定する．この2つのベクトルの内積は以下のようになる．
        ...
        であり，ベクトルの次元数の分だけ，内積の計算結果の分散が増大してしまう．分散が巨大になれば，ソフトマックス関数に大きな値が入力される確率が高くなる．そこで，内積の結果の分散を1に規格化するという操作を行うのである．

        $$\text{Var}\left(\frac{q \cdot k}{\sqrt{d_k}}\right) = \frac{\text{Var}(q \cdot k)}{(\sqrt{d_k})^2} = \frac{d_k}{d_k} = 1$$

    - 分割されたヘッドのConcatの後に線形層がある理由：
...
- LayerNormに初めまして
    - そもそも正規化の目的は？
        ...
        **「出力のスケールを安定させたい．」**
        ...
        ここまではBatchNormもLayerNormも同じ．
    
    - 何を正規化するかが異なる！

        - BatchNorm：特徴1は特徴1同士で比べる．各特徴は独立した「測定項目」だと思っている．
        - LayerNorm：単一サンプルを見て，特徴方向に統計量を計算する．そのサンプルの中で値が大きすぎる特徴を抑える．

        見ている対象は全然違う．でも，出力のスケールを安定させるという目的は全く同じなのだ．
    
    - LayerNormの貢献は？

        BatchNormと基本同じで「損失関数の地形を滑らかにすること」らしい．原論文のLayerNormの挿入位置には改善の余地があり，原論文以降にスタンダードな設計が変化している模様である．

- なぜEncoderブロックは，MHSAとMLPの2段構成なのか？
    - MHSA：トークン間（パッチ間・単語間）の関係を学習する．（Token Mixing）
    - MLP：各トークン内部の特徴を組み合わせて表現を豊かにする．（Channel Mixing）
...
### Attention Rollout
Attention Rolloutとは，「CLSトークンの家系図をたどって，最終的に入力画像のどのパッチがどれだけ先祖として寄与したかを計算する方法」である．でも，ヘッド方向に平均する処理が入っている事により，関係ないパッチが強調される場合があるので，Transformer Explainabilityの方が良いよ．

- 星のカービィの惑星から来たCLSのルーツを暴く．
    > 最終的なCLSに聞く。
    > 
    > 👤「あなた、どこから情報をもらったの？」
    > 
...
### ViTが捉えているモノ
- Transformerベースのモデル：形状に依って物体を認識
- CNNベースのモデル：テクスチャに依って物体を認識
- ヒト：形状に依って物体を認識

### アーキテクチャごとの「局所性」と「大域性」
- CNN構造：

    局所も大域も見れているように感じるが，畳み込み層が深くなると，出力のテンソルに局所情報は残りづらい．局所情報と大域情報を両方持ち続けることが困難である．わざわざU-Netを使って浅い層から局所情報を持ってこないとセグメンテーションがうまくいかなかったのは，そのせいである．

- Transformer構造：

    空間解像度を落とすこともなく，Self-Attentionに依って，第1層目から局所情報と大域情報の両方を捉えられる．

- 同じ「大域情報」でも性質が異なる．

    CNN構造は局所的な空間関係という帰納バイアスを持ち、階層的な特徴抽出によって空間解像度を下げながら大域的な情報を獲得する。一方、Transformer構造はSelf-Attentionにより、高い空間解像度を維持したまま全パッチ間の大域的な関係を直接モデル化できる。
    
    > ### 1. U-Netの「ぼやっとした大域」の学術的表現
    > 
    > あなたの言う「ピクセル周りに拡大するぼやっとした大域」は、学術的には以下の3つのキーワードで説明されます。
    > 
    > ① 局所受容野の階層的拡大 (Hierarchical expansion of local receptive fields)CNNは、$3 \times 3$ などの小さなフィルターを何層も重ねることで、徐々に見える範囲（受容野：Receptive Field）を広げていきます。いきなり全体を見るのではなく、「隣の隣の隣…」と伝言ゲームのように広がるため、どうしても境界が曖昧になります。
    > 
    > ② 距離に依存した減衰 (Distance-dependent information decay)伝言ゲームである以上、遠くのピクセルの情報ほど何回も畳み込み演算（足して混ぜる操作）を経由するため、情報が平滑化（スムージング）され、元の特徴が薄まってしまいます。これが「ぼやっとする」の正体です。
    > 
    > ③ 静的な重み付け (Static weight sharing)CNNのフィルターの重みは、入力画像が何であろうと固定（静的）です。入力の文脈に合わせて「ここは強く見よう」と動的に変化させることができないため、メリハリをつけるのが苦手です。
    > 
    > ### 2. ViTの「メリハリある大域」の学術的表現あなたの言う「ココとココッ！というメリハリある大域」は、学術的には以下の3つのキーワードで説明されます。
    > 
    > ① 長距離依存性の直接的モデリング (Direct modeling of long-range dependencies)ViTは第1層目から、どれだけ物理的な距離が離れていようと $QK^T$ という「たった1回の内積計算」でダイレクトに情報を繋ぎます。間に伝言ゲームを挟まないため、遠くの情報でも鮮度が落ちず、ダイレクトに結びつきます。
    > 
    > ② 距離に依存しない相互作用 (Distance-agnostic interaction)Self-Attentionの計算式には「ピクセル間の距離」という変数が存在しません（位置埋め込みは足しますが、内積計算自体は順不同です）。そのため、すぐ隣のパッチだろうが、画面の対角線上にあるパッチだろうが、平等に「ココとココ！」と強い関連性を持つことができます。
    > 
    > ③ コンテンツベースの動的重み付け (Content-based dynamic routing / weighting)これが最大のポイントです。ViTは「入力されたパッチの中身（コンテンツ）」同士の類似度を計算して重み（Attention Score）を決めるため、入力画像に応じて「強く注目する場所」と「無視する場所（ゼロに近い値）」が動的かつスパース（疎）に決定されます。これが「メリハリ」の正体です。

### ViTの文脈で「パッチサイズ」は何を指す？
「パッチサイズ」は，1枚のパッチの縦横サイズ（解像度）のこと．分割数ではない．
